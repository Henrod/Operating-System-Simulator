...
public class CentralMemory {
	 ...
	 */
	public boolean allocateSegment(jobTable job, int segment_id){
		int space = job.segmentList.search(segment_id).get_space();
		...
		if(freeSpace != null){
			...
			int job_final_address = job_initial_address + job.segmentList.search(segment_id).get_space() - 1;
			jobCM newJob = new jobCM(job_initial_address, job_final_address, job, segment_id);
	...
	public void displayStatus(){
		...
		for(emptyCM e = cmEmptyLinkedList.getFirstEmptyCM(); e != null; e = e.nextEmptyCM){
			System.out.println("	" + e.get_initial_address()/1000000.0 + " MB - " + e.get_final_address()/1000000.0 + " MB");
		}
		
		System.out.println("Allocated spaces: ");
		for(jobCM j = cmJobLinkedList.getFirstJobCM(); j != null; j = j.nextJobCM){
			System.out.println("	" + j.get_initial_address()/1000000.0 + " MB - " + j.get_final_address()/1000000.0 + " MB: Segment " + j.get_segment_id() + " Job " + j.getJob().get_job_id());
		}
		System.out.println("------------------------");
	}
}